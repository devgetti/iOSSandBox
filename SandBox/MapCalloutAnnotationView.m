...
- (void)animateIn {
	...
	self.transform = CGAffineTransformMake(scale, 0.0f, 0.0f, scale, [self xTransformForScale:scale endFrame:self.frame], [self yTransformForScale:scale endFrame:self.frame]);
...
- (CGFloat)xTransformForScale:(CGFloat)scale endFrame:(CGRect)endFrame {
	CGFloat xDistanceFromCenterToParent = endFrame.size.width / 2 - [self relativeParentXPosition];
	return (xDistanceFromCenterToParent * scale) - xDistanceFromCenterToParent;
}

- (CGFloat)yTransformForScale:(CGFloat)scale endFrame:(CGRect)endFrame {
	CGFloat yDistanceFromCenterToParent = (((endFrame.size.height) / 2) + self.offsetFromParent.y + CalloutMapAnnotationViewBottomShadowBufferSize + CalloutMapAnnotationViewHeightAboveParent);
